...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="Document">
		...
		<xsl:choose>
			<xsl:when test="Format = 'SMALL'">
				<xsl:apply-templates select="Post" mode="small"/>
			</xsl:when>
			<xsl:when test="Format = 'LARGE'">
				<xsl:apply-templates select="Post" mode="large"/>
			</xsl:when>			
			<xsl:otherwise>
				<xsl:apply-templates select="Post" mode="email"/>
			</xsl:otherwise>
		</xsl:choose>
		
	</xsl:template>
	
	<xsl:template match="Post" mode="email">
		...
		<li>
			...
			<xsl:value-of select="$i18n.HasAddedPost"/>
	...
	<xsl:template match="Post" mode="small">
	...
	<xsl:template match="Post" mode="large">
