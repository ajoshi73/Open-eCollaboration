...
+ function($) {
    ...
    $(document).ready(function() {
        ...
        $("table[data-of-sortable]").each(function() {

            var table = $(this),
                rows = table.find('tbody tr'),
                clones = table.parent().find('.of-table-clones i'),
                select = $('<select/>').addClass('of-hide-from-md'),
                option = $('<option/>');
            ...
            if (rows.length > 1) {
                ...
                var sortColumns = table.find('thead tr th'),
                    sortColumn = table.find('thead tr th[data-of-default-sort]'),
                    noSort = table.find('thead tr th.no-sort'),
                    headers = {};

                sortColumns.each(function(i) {

                    var col = $(this),
                        optAsc = option.clone();

                    optAsc.val(i + ', a');
                    optAsc.html(col.text() + ' &uarr;');
                    select.append(optAsc);

                    var optDesc = option.clone();
                    optDesc.val(i + ', d');
                    optDesc.html(col.html() + ' &darr;');
                    select.append(optDesc);


                    col.prepend(clones.clone());

                });

                select.on('change', function(e) {
                    var selected = select.find('option:selected'),
                        val = selected.val().split(', ');
                ...
                table.parent().find('.of-table-clones').hide();
                select.data('of-select', 'inline').insertBefore(table);
