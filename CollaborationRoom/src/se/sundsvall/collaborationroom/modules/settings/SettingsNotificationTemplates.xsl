...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template name="NameChangeEmail">
		...
		<li>
			...
			<xsl:value-of select="$i18n.HasChangedSectionNameFrom"/>
			
			<xsl:text>&#160;</xsl:text>
			
			<a href="{ContextPath}/{SectionID}"><xsl:value-of select="OldName"/></a>
			
			<xsl:text>&#160;</xsl:text>
			
			<xsl:value-of select="$i18n.HasChangedSectionNameTo"/>
			
			<xsl:text>&#160;</xsl:text>
			
			<a href="{ContextPath}/{SectionID}"><xsl:value-of select="NewName"/></a>
	...
	<xsl:template name="NameChangeList">
		...
		<li>
			<a class="of-icon" href="{ContextPath}/{SectionID}">
				<i>
