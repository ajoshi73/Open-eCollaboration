...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="Document">
		...
		<xsl:choose>
			<xsl:when test="Format = 'SMALL'">
				<xsl:apply-templates select="CalendarPost" mode="small"/>
			</xsl:when>
			<xsl:when test="Format = 'LARGE'">
				<xsl:apply-templates select="CalendarPost" mode="large"/>
			</xsl:when>			
			<xsl:otherwise>
				<xsl:apply-templates select="CalendarPost" mode="email"/>
			</xsl:otherwise>
		</xsl:choose>
		
	</xsl:template>
	
	<xsl:template match="CalendarPost" mode="email">
		...
		<li>
			...
			<xsl:value-of select="$i18n.HasAddedCalendarPost"/>

			<xsl:text>&#160;</xsl:text>

			<a href="{../PostURL}"><xsl:value-of select="title"/></a>
	...
	<xsl:template match="CalendarPost" mode="small">
	...
	<xsl:template match="CalendarPost" mode="large">
			...
			<article>
				...
				<ul class="of-meta-line">
					<li class="of-icon">
						...
						<xsl:value-of select="$i18n.CalendarPost"/>
					</li>
